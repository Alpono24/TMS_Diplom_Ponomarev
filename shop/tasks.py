...
def update_currency_rate_usd_nbrb():
    ...
    try:
        ...
        try:
            ...
            print("Ошибка: отсутствует ключ Cur_OfficialRate в ответе.")
            price_logger.error("Ошибка: отсутствует ключ Cur_OfficialRate в ответе.")
            return
        except ValueError:
            print("Ошибка: невозможно преобразовать значение Cur_OfficialRate в Decimal.")
            price_logger.error("Ошибка: невозможно преобразовать значение Cur_OfficialRate в Decimal.")
            return
        ...
        price_logger.error(f"Ошибка таймаута: {te}")
    except MaxRetryError as mre:
        print(f"Превышено максимальное число повторных запросов: {mre}")
        price_logger.error(f"Превышено максимальное число повторных запросов: {mre}")
    except NewConnectionError as nce:
        print(f"Ошибка подключения: {nce}")
        price_logger.error(f"Ошибка подключения: {nce}")
    except RequestException as re:
        print(f"Общая ошибка запроса: {re}")
        price_logger.error(f"Общая ошибка запроса: {re}")
    except Exception as ex:
        print(f"Необработанная ошибка: {ex}")
        price_logger.error(f"Необработанная ошибка: {ex}")
...
def update_currency_rate_eur_nbrb():
    """
    Задача для автоматической конвертации Евро в белорусские рубли на основе курса Нацбанка РБ.
    Выполняется каждый час.
    """
